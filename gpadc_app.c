...
void gpadc_app_task(void *pvParameters)
{
    ...
    static float    mv0;   // float: correct_mv() returns float
    static float    mv1;
    int ret = 0;
    uint16_t dummy = 0;
     ...
     * The dummy read is meant to let the sample-and-hold
     * capacitor settle after the mux switches to a new input.
     */
    ...
    /* --- An infinity loop that read sequentially Ch0 and Ch1 --- */
    for (;;) {
        ...
        mv0 = (ret == AD_GPADC_ERROR_NONE) ? correct_mv(ad_gpadc_conv_to_mvolt(CHAN0_DEVICE->drv, raw0), OFFSET_MV_CH0, GAIN_CH0) : 0.0f;
        ...
        mv1 = (ret == AD_GPADC_ERROR_NONE) ? correct_mv(ad_gpadc_conv_to_mvolt(CHAN1_DEVICE->drv, raw1), OFFSET_MV_CH1, GAIN_CH1) : 0.0f;
        ad_gpadc_close(h1, false);

        /* Print two integers only — fast, low UART load */
        printf("%d,%d\n", (int)mv0, (int)mv1);
